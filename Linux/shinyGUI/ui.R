...
shinyUI(navbarPage(
  ...
  tabPanel(
    ...
    sidebarLayout(
      sidebarPanel(
        numericInput(
          ...
          min = 0,
          value = 50,
          label = "Median coverage threshold for failed samples"
        ),
        numericInput(
          "SampleCorLimit",
          min = 0,
          max = 1,
          value = .98,
          label = "Correlation threshold for failed samples",
          step = .01
        )
      ),
      mainPanel(dataTableOutput("FailedSamples"))
    ),
    ...
    sidebarLayout(sidebarPanel(
      numericInput(
        ...
        min = 0,
        value = 50,
        label = "Median coverage threshold for failed exons"
      )
    ),
    mainPanel(dataTableOutput("FailedExons"))),
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        radioButtons(
          "ChooseFrom",
          choices = list("Reference Samples" = 1, "All Samples" = 2),
          label = "Choose From: "
        ),
        ...
        radioButtons(
          "plotType",
          choices = list("Box plot" = 1, "Line plot" = 2),
          label = "Type of plot"
        ),
        radioButtons(
          "plotScale",
      ...
      mainPanel(plotOutput("CovPlot", height = 700))
    )
  ),
  ...
  tabPanel(
    ...
    h2('Welcome!'),
    p(
      'This web app provides an interface to the results of running DECoN on your data. You can check the data which has been used, evaluate the coverage of individual exons and individual samples and visualise the CNV calls.'
    ),
    p(
      'The data shown is from the input file provided, generated by DECoN version 1.0.1. If it\'s not what you\'re expecting, then first check that this is the correct file and that it has been generated according to the documentation.'
    ),
    p(
      'This program requires that you have a modern internet browser (Firefox, Chrome or IE10) and R installed.'
    )
  )
  # tabPanel("Details")
))
